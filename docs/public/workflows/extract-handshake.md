...
For several generic source types, or for a reusable APB, ATB, AXI, or AXI-Stream profile/mapping setup, write a source file and pass `--source`. Use `wavepeek schema --input` to fetch the exact input schema for that file.
