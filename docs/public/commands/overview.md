...
Use `extract` commands when you want one row per matching synchronous event with ordered payload values sampled at the pre-edge point. `extract apb` covers APB3, APB4, and APB5 Setup, waited Access, and completed Access states from Arm IHI 0024E Issue E. `extract atb` covers AMBA ATB transfer, flush, and synchronization-request events. `extract axi` covers AXI3, AXI4, AXI4-Lite, AXI5, AXI5-Lite, ACE, ACE-Lite, ACE5, ACE5-Lite, ACE5-LiteDVM, and ACE5-LiteACP ready/valid channel transfers. `extract axistream` covers one AXI4-Stream or AXI5-Stream interface, including an explicit mode for physically omitted `TREADY`. `extract generic` covers custom handshakes, FIFO pushes and pops, and other transfer-like rows.
