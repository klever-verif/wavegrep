...
Pre-commit configuration lives in `.pre-commit-config.yaml`, and the reviewed host dispatcher lives at `tools/repo/git-hook`. `./dev --install-hooks` copies the dispatcher and `./dev` into host data storage that is not mounted in the container; hooks then use only `./dev --exec-only` and require the invoking worktree's container to be running. Hooks should stay deterministic, non-interactive, and wired through `just` where possible.
