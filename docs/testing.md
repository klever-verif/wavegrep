...
Tests should prove the public command and machine-output contracts, not just exercise code paths. The stable behavior references are `skills/wavepeek/references/command-model.md`, `skills/wavepeek/references/machine-output.md`, and `skills/wavepeek/references/expression-language.md`.
...
`./dev just playground-test` builds Rust/WASM and the Material page, then uses headless Chromium to compare browser and native human/JSON/JSONL output and status. It also checks AXI FST streaming, unsupported browser options, editable-command synchronization, local-file privacy, reload and Stop behavior, Surfer linking, and responsive layout.
