...
- identify dump bounds, time resolution, hierarchy, scopes, or signal names;
- sample values at known timestamps;
- inspect changes or clock-aligned state over a time window;
- find when a Boolean condition matches, asserts, or deasserts;
- extract generic synchronous events with payloads;
- inspect AXI, AXI-Stream, AHB, APB, or ATB transfers and phase events;
- consume Wavepeek JSON or JSONL in another script.
...
- [Overview](references/overview.md): purpose, operating model, and the full documentation entry points.
