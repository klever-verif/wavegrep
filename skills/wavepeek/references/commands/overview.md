...
Read [Machine output](../reference/machine-output.md) when a client needs JSON envelope or JSONL stream details.
...
Use `change` to inspect value transitions across a bounded time range. Trigger selection comes from `--on`; expression syntax is documented in [Expression language](../reference/expression-language.md).
...
When choosing between VCD, FST, and FSDB input or diagnosing unexpectedly slow queries, use [Waveform performance](../reference/waveform-performance.md) for format-level performance guidance.

## Which document is normative?

Use this overview to choose a command quickly. When exact flags matter, defer to generated help. For behavioral semantics, use the [Command model](../reference/command-model.md). For exact JSON and JSONL shapes, use [Machine output](../reference/machine-output.md).
