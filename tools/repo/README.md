...
`git-hook` is the reviewed source for host `pre-commit` and `commit-msg` dispatch. Install copies in host-only data storage with `./dev --install-hooks`; do not execute the tracked source directly.
