...
`git-hook` is the reviewed source for host `pre-commit` and `commit-msg` dispatch. Install copies outside tracked worktrees with host `./dev --install-hooks`; do not execute the tracked source directly.
